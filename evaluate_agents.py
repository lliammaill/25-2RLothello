# evaluate_agents.py
# (경로: /Users/liam/Desktop/Othello/evaluate_agents.py)

import numpy as np
import pickle
import os
import torch
from othello_env_agent import OthelloEnv, QLearningAgent, DQNAgent, DQNetwork 

# ============================================================
# 로컬 환경 설정
# ============================================================
PROJECT_DIR = '/Users/liam/Desktop/Othello'
Q_TABLE_LOAD_PATH = os.path.join(PROJECT_DIR, "q_table_othello.pkl")
MODEL_LOAD_PATH = os.path.join(PROJECT_DIR, "dqn_othello_model.pth")

# M1/MPS 디바이스 설정 (DQN 로드 시 필요)
if torch.backends.mps.is_available():
    DEVICE = torch.device("mps")
elif torch.cuda.is_available():
    DEVICE = torch.device("cuda")
else:
    DEVICE = torch.device("cpu")


# ============================================================
# 1️⃣ Agent 로드 함수
# ============================================================
def load_q_agent(env):
    """Q-Table을 로드하여 QLearningAgent를 생성합니다."""
    try:
        with open(Q_TABLE_LOAD_PATH, 'rb') as f:
            q_table_data = pickle.load(f)
        
        # QLearningAgent 객체 생성 (epsilon=0으로 설정하여 탐욕적으로 만듦)
        q_agent = QLearningAgent(env, epsilon=0.0)
        
        # 로드된 dict 데이터를 defaultdict로 다시 변환하여 할당
        for state_key, q_values in q_table_data.items():
            q_agent.q_table[state_key] = q_values
            
        print(f"✅ Q-Learning Agent 로드 성공. (Q-Table 크기: {len(q_agent.q_table)})")
        return q_agent
    except FileNotFoundError:
        print(f"❌ 오류: Q-Table 파일이 없습니다. Q-Learning Agent를 학습(train_q_learning.py)하세요.")
        return None
    except Exception as e:
        print(f"❌ 오류: Q-Learning Agent 로드 중 오류 발생: {e}")
        return None


def load_dqn_agent(env):
    """모델 가중치를 로드하여 DQNAgent를 생성합니다."""
    try:
        dqn_agent = DQNAgent(env, epsilon=0.0)
        
        # 학습된 모델 가중치 로드
        dqn_agent.q_net.load_state_dict(torch.load(MODEL_LOAD_PATH, map_location=DEVICE))
        dqn_agent.q_net.eval() # 평가 모드 설정 (Dropout 등을 비활성화)
        
        print("✅ DQN Agent 로드 성공.")
        return dqn_agent
    except FileNotFoundError:
        print(f"❌ 오류: DQN 모델 파일이 없습니다. DQN Agent를 학습(train_dqn.py)하세요.")
        return None
    except Exception as e:
        print(f"❌ 오류: DQN Agent 로드 중 오류 발생: {e}")
        return None


# ============================================================
# 2️⃣ 대결 함수
# ============================================================
def play_game(env, player1, player2, starting_player=1):
    """두 에이전트 간의 게임을 플레이하고 승자(1 또는 2)를 반환합니다."""
    
    # gym reset은 (state, info)를 반환하므로, 상태(state)만 받습니다.
    state, _ = env.reset() 

    current_player_id = starting_player
    
    # 흑(1)과 백(2) 에이전트 매핑
    agents = {1: player1, 2: player2}
    
    terminated = False
    truncated = False
    
    while not (terminated or truncated):
        current_agent = agents[current_player_id]
        env.current_player = current_player_id # 환경의 현재 플레이어 설정
        
        valid_moves = env.valid_moves()
        
        if not valid_moves:
            # 둘 곳이 없으면 상대방에게 턴을 넘김
            opponent_id = 3 - current_player_id
            env.current_player = opponent_id
            
            # 상대방도 둘 곳이 없으면 게임 종료
            if not env.valid_moves():
                env.game_over = True
                terminated = True
                break
            
            # 상대방에게 턴을 넘김
            current_player_id = opponent_id
            continue

        action = current_agent.act(state, valid_moves)
        
        # ⭐⭐ 5가지 반환 값 받기 (next_state, reward, terminated, truncated, info) ⭐⭐
        next_state, reward, terminated, truncated, _ = env.step(action)
        
        state = next_state
        current_player_id = 3 - current_player_id
        
    # 게임 종료 후 승자 결정
    black_score = np.sum(env.board == 1)
    white_score = np.sum(env.board == 2)
    
    if black_score > white_score:
        return 1 # 흑 승리
    elif white_score > black_score:
        return 2 # 백 승리
    else:
        return 0 # 무승부


# ============================================================
# 3️⃣ 메인 실행 루프
# ============================================================
if __name__ == '__main__':
    env = OthelloEnv()
    
    q_agent = load_q_agent(env)
    dqn_agent = load_dqn_agent(env)

    if q_agent is None or dqn_agent is None:
        print("에이전트 로드 실패. 대결을 진행할 수 없습니다.")
    else:
        N_GAMES = 100
        dqn_wins = 0
        q_wins = 0
        draws = 0
        
        print(f"\n======== ⚔️ DQN vs Q-Learning 대결 시작 (총 {N_GAMES}판) ========")
        print("DQN(1) vs QL(2)은 50판, QL(1) vs DQN(2)은 50판 진행합니다.")

        for i in range(N_GAMES):
            # 50판은 DQN이 선공(1), QL이 후공(2)
            if i < N_GAMES / 2:
                player1, player2 = dqn_agent, q_agent
                start_id = 1
                p1_name, p2_name = "DQN", "QL"
            # 나머지 50판은 QL이 선공(1), DQN이 후공(2)
            else:
                player1, player2 = q_agent, dqn_agent
                start_id = 1
                p1_name, p2_name = "QL", "DQN"
                
            winner = play_game(env, player1, player2, starting_player=start_id)

            if winner == 1:
                if p1_name == "DQN":
                    dqn_wins += 1
                else:
                    q_wins += 1
            elif winner == 2:
                if p2_name == "DQN":
                    dqn_wins += 1
                else:
                    q_wins += 1
            else:
                draws += 1
                
            if (i + 1) % 10 == 0:
                print(f"[{i+1}/{N_GAMES} 게임 완료] DQN 승: {dqn_wins}, QL 승: {q_wins}, 무승부: {draws}")


        # 최종 결과 출력
        total_wins = dqn_wins + q_wins
        dqn_win_rate = (dqn_wins / total_wins) * 100 if total_wins > 0 else 0
        ql_win_rate = (q_wins / total_wins) * 100 if total_wins > 0 else 0
        
        print("\n\n===================== 🏆 최종 결과 🏆 =====================")
        print(f"총 게임 수: {N_GAMES} (선후공 교대 포함)")
        print(f"DQN Agent 승리: {dqn_wins}회 ({dqn_win_rate:.2f}%)")
        print(f"Q-Learning Agent 승리: {q_wins}회 ({ql_win_rate:.2f}%)")
        print(f"무승부: {draws}회")
        print("==========================================================")